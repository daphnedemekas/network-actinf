...
initial_spins = (initial_posteriors > 0.5).astype(
    float
...
# A = nx.to_numpy_array(G)
phi_hist, spin_hist, _, _ = run_simulation(
    1000, initial_spins, initial_posteriors, A, 0.99, 0.5
)
...
phi_hist, spin_hist, _, _ = run_simulation(
    T, initial_spins, initial_posteriors, A, 0.50001, 0.5
)
...
initial_spins = (initial_posteriors > 0.5).astype(float)
phi_hist, spin_hist, _, _ = run_simulation(
    T, initial_spins, initial_posteriors, A, 0.601, 0.5
)
...
initial_spins = (initial_posteriors > 0.5).astype(float)
phi_hist, spin_hist, _, _ = run_simulation(
    T, initial_spins, initial_posteriors, A, 0.75, 0.5
)
...
for (ii, po) in enumerate(po_vec):
    ...
    for trial_i in range(n_trials):
        ...
        initial_posteriors = np.random.rand(N)
        initial_spins = (initial_posteriors > 0.5).astype(float)
        phi_hist, spin_hist, kld_hist, accur_hist = run_simulation(
            T, initial_spins, initial_posteriors, A, po, ps
        )
...
axes[1].set_yticks(ticks=np.linspace(-3.0, 3.0, 4), fontsize=16)
...
for (ii, po) in enumerate(po_vec):
    ...
    for trial_i in range(n_trials):
        initial_posteriors = np.random.rand(N)
        initial_spins = (initial_posteriors > 0.5).astype(float)
        phi_hist, spin_hist = run_simulation(
            T, initial_spins, initial_posteriors, A, po, ps
        )
