...
from simulation import SimulationNP
...
def run():
    ...
    sim = SimulationNP(G, omegas, ps_vec)
...
def run_learning():
    ...
    sim = SimulationNP(G, k_matrix, p_s_vec = ps_vec, init_scale = 1.0)

    phi_hist, spin_hist, k_matrix_hist = sim.run(T)
    # print(phi_hist.shape,phi_hist)
    # print(spin_hist.shape,spin_hist)
    ...
    fig, axes = plt.subplots(len(focal_agents), len(observed_agents), figsize = (10, 4), dpi = 100)
