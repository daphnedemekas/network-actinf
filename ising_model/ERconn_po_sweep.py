...
def run_sweep(param_combos, num_trials, T, run_name):
    ...
    for (param_idx, param_combo_i) in enumerate(param_combos):
        ...
        for trial_i in range(num_trials):
            ...
            avg_complexity_per_trial[trial_i] = complexity[
                :, 100:
            ].mean()  # exclude transient
            avg_neg_accur_per_trial[trial_i] = neg_accur[
                :, 100:
            ].mean()  # exclude transient
            avg_polarization_per_trial[trial_i] = phi_hist[
                :, 100:
            ].mean()  # exclude transient
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--network_size", "-N", type=int, help="Size of network", dest="N"
    )
    ...
    starting_p = (
        1.5 * np.log(N) / N
    )  # log(N)/N is the value of p where network is connected about 36% of the time
